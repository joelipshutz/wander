...
REC-82 introduced a preferred Google Places image through the authenticated `place-photo` Edge Function. The backend capability still validates the Clerk/Supabase bearer token through the existing `current_profile` PostgREST contract, checks the private `google-place-photo-cache` bucket, records Google Maps/author/source attribution, and returns a short-lived signed Storage URL. REC-340 stops requesting or displaying those provider images in Featured/For You, Activity, Place Profile, collapsed place cards, Lists, and import-review thumbnails. Those shared previews now use visible visit photos or rec.me/category artwork, so the Google attribution watermark is not rendered there.

iOS now calls `public.first_visible_place_photo(place_id)` as the shared-preview photo source. That security-invoker RPC returns the earliest uploaded visit-photo object allowed by existing user-place/visit/photo RLS. The image bytes are downloaded from the private `visit-photos` bucket with the signed-in user's auth headers. This makes the first uploaded photo the shared default for places and dropped pins that friends can see, without exposing a permanent public URL. A just-added local photo renders immediately from its local asset while upload is pending. MapKit/category artwork is the fallback when no visible user photo exists.

With the REC-82 field mask, Google currently bills venue matching as Text Search Pro, which has a separate 5,000-event monthly free cap; Place Details Photos has a 1,000-event monthly free cap. Most rec.me places originate in MapKit, so the first uncached photo open can consume one event from each SKU; a valid server-cache hit consumes neither. As of 2026-08-13, the Google Cloud account uses paid billing and a $50 monthly budget alert. The Edge Function no longer calls the legacy quota RPC, and there is no global or per-user application-side usage cap. Google Cloud budget alerts are advisory and do not stop spend automatically, so billing reports and alerts are the operational monitoring source of truth. Coordinate/dropped pins bypass Google and go directly to the visible user-photo fallback. Keep Places API (New) method quotas and budget alerts configured, and restrict the server-side key to Places API. Yelp is not a free commercial fallback: its free access is a 30-day evaluation trial.
...
The function and `first_visible_place_photo` RPC were deployed to the linked rec.me project on 2026-07-12. The user-photo fallback is live. `WANDER_GOOGLE_PLACES_API_KEY` was installed as a managed Supabase secret and `place-photo` was redeployed on 2026-07-12. A live Text Search plus Place Details Photo check matched Ronan and returned an attributed Google media URL. The former hard quota was removed on 2026-08-13 after paid Google Cloud billing and a $50 monthly budget alert were activated. Keep the Google Cloud key restricted to Places API (New), retain provider-side quotas and budget alerts, and rotate the credential if it is ever exposed outside approved secret storage.

Google's Places policy restricts caching and warns that photo resource names expire. Do not store photo resource names or Google-hosted media URLs. At Ryan's direction on 2026-08-14, rec.me explicitly accepted the policy risk of keeping the private server-side image cache indefinitely. The cache keeps original image bytes unmodified, retains the required attribution/source link, and has no automatic expiration; cached objects remain until deliberately removed or replaced. The legacy nullable `expires_at` column is retained only for migration compatibility and is not read or written by the Edge Function. The iOS app keeps no persistent provider-photo cache. Google Place IDs may be retained. Any future UI that displays Google content must restore the Google Maps attribution, photo author attribution when present, and source-photo link with the image.
