...
Google's Places policy restricts caching and warns that photo resource names expire. Do not store photo resource names or Google-hosted media URLs. The server cache is private, keeps original image bytes unmodified, retains the required attribution/source link, and sets `expires_at` no later than 30 days after fetch. An expired entry is deleted and refreshed on its next request. The iOS app still keeps no persistent provider-photo cache. Google Place IDs may be retained. The UI must keep the Google Maps attribution, photo author attribution when present, and source-photo link visible with the image.
