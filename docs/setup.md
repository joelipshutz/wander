...
Google's Places policy restricts caching and warns that photo resource names expire. Do not store photo resource names or Google-hosted media URLs. At Ryan's direction on 2026-08-14, rec.me explicitly accepts the policy risk of keeping the private server-side image cache indefinitely. The cache keeps original image bytes unmodified, retains the required attribution/source link, and has no automatic expiration; cached objects remain until deliberately removed or replaced. The legacy nullable `expires_at` column is retained only for migration compatibility and is not read or written by the Edge Function. The iOS app still keeps no persistent provider-photo cache. Google Place IDs may be retained. The UI must keep the Google Maps attribution, photo author attribution when present, and source-photo link visible with the image.
