...
REC-82 resolves one preferred image for both the full place-profile header and collapsed map card. The iOS app first calls the authenticated `place-photo` Edge Function, which validates the Clerk/Supabase bearer token through the existing `current_profile` PostgREST contract. The function checks the private `google-place-photo-cache` bucket before Google. A cache miss matches the place by provider id or name plus coordinates, requests Google's first returned usable photo, downloads the bytes server-side, and stores them under a SHA-256 lookup key. It records the Google Maps/author/source attribution in the service-role-only `google_place_photo_cache` table and returns a 24-hour signed Supabase Storage URL. Subsequent app installs and users reuse that object without repeating Google Search and Photo calls. Google does not expose a storefront/signage label or usage count, so this is a best available default rather than a guaranteed exterior photo.
...
Google's Places policy restricts caching and warns that photo resource names expire. Do not store photo resource names or Google-hosted media URLs. At Ryan's direction on 2026-08-14, rec.me explicitly accepts the policy risk of keeping the private server-side image cache indefinitely. The cache keeps original image bytes unmodified, retains attribution/source metadata, and has no automatic expiration; cached objects remain until deliberately removed or replaced. The legacy nullable `expires_at` column is retained only for migration compatibility and is not read or written by the Edge Function. The iOS app still keeps no persistent provider-photo cache. Google Place IDs may be retained. REC-340 keeps Google provider photos but removes the visible Google Maps/Google watermark labels, logo treatment, and provider-only gallery attribution card from shared preview UI. Ryan explicitly accepted the resulting attribution/provider-policy risk on 2026-08-25; do not interpret the stored attribution metadata as visible UI attribution.
