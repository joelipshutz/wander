...
| Unified save-place tags | Locked for REC-155 design; implementation pending | The save flow will merge place tags and `My Labels` into one user-facing field named **Tags**. It presents one normalized, case-insensitively deduplicated suggestion set and recomputes it when place taxonomy changes: category + cuisine for Restaurants & Food, or category + subcategory otherwise. Suggestions must stay taxonomy-coherent; changing a Mediterranean restaurant to another cuisine replaces incompatible cuisine-specific options while preserving still-valid selections and custom tags. The approved selector direction remains to be chosen from the REC-155 static mockups; persistence/schema migration is not authorized by the design pass. |
