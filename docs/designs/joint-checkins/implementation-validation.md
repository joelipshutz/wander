...
The earlier full regression run completed 2,633 tests: 2,604 passed and 29 failed (2,409 unit and 224 UI). Its two failures in the changed area—a source-contract expectation using the old context name and a small expansion accessibility frame—are fixed and pass in focused runs. The other **27 UI failures remain unresolved** and have not been reproduced on an unchanged baseline; do not label them pre-existing or count the full run as passing. Local `native-full-failure-triage.md` lists every failure. The PR remains draft. Verbose simulator diagnostic collection repeatedly timed out after execution; final focused runs disable only that optional collection, retaining assertions, logs, screenshots and result bundles.
...
- Broader regression triage and final live-backend detail/history parity beyond the passing feed/profile fixtures.
