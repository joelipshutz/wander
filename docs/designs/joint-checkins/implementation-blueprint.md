...
This makes the [product and compatibility contract](engineering-plan.md) buildable. Joe requested the feature, then an engineering plan first, and explicitly requested the linked `plan-eng-review` skill. The planning review below is preserved as its historical record. Implementation now accompanies it; [implementation validation](implementation-validation.md) records the tested revision and remaining rollout gates. The [88-case matrix](test-plan.md) is the acceptance contract; the exact-skill pass added three deletion/rejoin/composer cases.
...
The implementation splits these additive changes into six ordered migrations (`20260921220000` through `20260921225000`) so identity, mutations, lifecycle, projections, engagement and compatibility can be reviewed separately:
...
| `edit_joint_check_in` | Owned visit/answers payload, group ID, exact displayed `updated_at`, stable operation UUID | Compare-and-swap only this contribution; stale editors retain their draft. Retry returns the committed receipt without a second edit. |
...
| `joint_check_in_contexts` | Up to 50 visit IDs; canonical activity lookup uses `activity_detail_v2` | Authorized mapping + deduplicated group DTOs for profile/history/detail; absent or closed mapping is explicit, never interpreted as “safe to use cached group”. |
...
REC-564 compatibility: `set_activity_comment_like_v2(comment_id, is_liked)` preserves likes on shared comments. It resolves the exact existing comment target, takes group/event locks before the comment lock, and rechecks current audience and author visibility. Comment pages and replayed post receipts retain like counts and viewer state. Legacy comment likes remain usable only for legacy discussions.
...
Legacy mutation RPCs reject a v2 group/member/canonical event with `joint_check_in_upgrade_required`, including invitation edits and engagement. Legacy social feed/detail/media readers exclude unsupported v2 projections; never return a half-populated owner-only shared thread. Owned v1 note/photo edits and deletes remain unchanged. Active v2 note/rating/answer edits require `edit_joint_check_in` and the exact displayed visit `updated_at` token; old clients receive an upgrade error instead of bypassing conflict detection. Existing owner-scoped photo operations and deletes remain usable with lifecycle reconciliation. Place/date edits after acceptance require the explicit detach/close path. New readers support v1 and v2; absence of the new RPC permits legacy fallback only for an exact missing-function response and only for operations with no v2 draft/identity. Auth, timeout and decoding errors are not capability detection.
