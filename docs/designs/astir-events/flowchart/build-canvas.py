...
tree=json.loads((root/'flowchart/decision-tree.json').read_text())
...
assert set(ids)=={n['screenId'] for s in tree['stages'] for n in s['nodes'] if n['type']=='screen'}
...
<div class="board-help-pop" id="help-pop" hidden><button id="close-help" aria-label="Close help">×</button><h2>A board you can move around.</h2><p>Drag the board or scroll with two fingers to pan. Pinch, or hold <kbd>Ctrl</kbd> while scrolling, to zoom around your pointer.</p><p>Click a preview to expand it in place. Double-click a screen to focus, or a decision to frame its branches. Nothing opens in another page.</p><p><kbd>/</kbd> Search · <kbd>+</kbd> / <kbd>−</kbd> Zoom<br><kbd>F</kbd> Focus screen · <kbd>Shift 1</kbd> Fit all<br><kbd>0</kbd> 100% · Arrow keys pan · <kbd>Esc</kbd> Clear</p></div>
<div id="canvas-toast" class="canvas-toast" role="status" hidden></div><div id="live-status" class="sr-only" aria-live="polite"></div>
'''+assets+'\n<script id="canvas-layout" type="application/json">'+json.dumps({'sections':model,'core':core,'short':short,'tree':tree},ensure_ascii=False).replace('</','<\/')+'</script>\n<script>\n'+(root/'flowchart/canvas-camera.js').read_text()+'\n'+(root/'flowchart/decision-layout.js').read_text()+'\n'+(root/'flowchart/canvas-review.js').read_text()+'\n</script></body></html>'
