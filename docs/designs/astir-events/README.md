...
Start with the **[Joe/Ryan engineering handoff](engineering-handoff.md)** for the two-agent work split, shared checkpoints and estimates. Open the **[review canvas](astir-events-flowchart.html)** to pan and zoom around all **172 screen states in 13 groups**, with previews visible and expandable in place. The guest journey runs left to right; offline admission, home-location rules and staff operations remain separate supporting branches.

Drag the board or use two-finger scrolling to pan; pinch or Ctrl+scroll to zoom. Double-click a screen to focus, click a preview to expand, or use search, Fit all and the minimap. The **[original linear view](astir-events-linear.html)** remains available for scrolling and print review.
...
The HTML has 26 primary native mocks and 146 wireframe previews; 32 native variants appear in comparisons. Native images are fixtures from a separate design sandbox, not production feasibility evidence. The linear HTML preserves the previously reviewed artifact byte for byte. The new canvas preserves all 172 screen contents and embedded assets; it only changes review navigation and reserves media dimensions to keep the board stable. [Coverage](flowchart/continuous-coverage.json), [preview checks](flowchart/inline-preview-verification.json), [offline changes](flowchart/offline-door-update-verification.json) and [approval annotations](flowchart/approval-annotations-update-verification.json) document artifact validation. Canvas source, asset preservation, camera gesture math and syntax checks passed; [canvas verification](flowchart/canvas-verification.json) records them. **The canvas has not passed a visual browser test:** the available browser blocked local-file access, and no workaround was attempted. The earlier browser checks apply to the preserved linear view. No Events runtime tests, benchmarks, migrations or releases were performed.
