## Main landed — final September 19 handoff

Onboarding + full founders VHS C are on remote main at **3a12911b2f3be9dd50399364fc07e68cb872fab5**. The clean primary `wander` checkout was fast-forwarded and its native C movie hash verified. Xcode is open on that main checkout, **Onboarding Review**, iPhone (254), for local fixture review. No new phone install is claimed. Final selected-C store/player/UI checks: **5/5 passed**, including Skip and actual end into NUX. Full earlier units 2,346 passed; final login/background check 1/1 passed. REC-547 is Done with exact main link. All 1,610 archived files hash-verified; source/media are published in Git/LFS.

**TestFlight 177 remains pending in REC-556.** Baseline testflight/build-176; frozen app cutoff 3a12911. Pre-bump reconciliation passes using the complete concise 3,713-character tester copy in `testflight-177-evidence/tester-copy.md`; do not drop manifest entries. No build-number bump/archive/upload/tag has occurred. Ignored release LocalAuth now has PostHog token/host exactly matching verified signed build176; check the final archive before upload.

**Build blocked by disk floor (~48 GiB / required50).** Automatic approval review rejected deletion of five legacy/unmarked caches. Read-only preflight proves all five unused, no release archives, 4 test results +24 dSYMs to preserve. Weekly cleanup preview found zero eligible. Explicit exception approval is pending in chat; do not bypass. Plan/script `/tmp/astir-release-legacy-cache-clean.py`; evidence `testflight-177-evidence/legacy-cache-preflight.json`. If approved, preserve results/symbols, recheck idle/open handles, clear only those five paths, then continue normal managed exact-candidate tests/archive and TestFlight workflow. If not approved, leave TestFlight pending and do not claim release.

Standing September19 ticket authorization resolved the earlier Linear rejection; routine updates are now permitted. Keep Ryan NUX untouched. No captions implemented. Full movie C selected, not any original/plain video or split-flap experiment.
