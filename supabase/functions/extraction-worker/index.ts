...
async function handleRequest(req: Request): Promise<Response> {
  ...
  if (jobId) {
    ...
    const payload = await authenticatedRpc<WorkerPayload>(
  ...
  for (let index = 0; index < limit; index += 1) {
    const payload = await serviceRpc<WorkerPayload | null>(
...
async function extract(source: SourceArtifact): Promise<ExtractionResult> {
  ...
  if (source.type === "url") {
    ...
    if (googleCandidate) {
      const candidate = await enrichCandidateCategory(
        googleCandidate,
        source,
        steps,
      );
    ...
    const metadataCandidate = await webMetadataCoordinateCandidate(
      resolvedURL,
      source,
      steps,
    );
...
function googleMapsCandidate(
  url: URL,
  source: SourceArtifact,
  steps: string[],
): ExtractedCandidate | null {
  if (!isGoogleMapsHost(url.hostname) && !isShortMapHost(url.hostname)) {
    return null;
  }
...
async function webMetadataCoordinateCandidate(
  url: URL,
  source: SourceArtifact,
  steps: string[],
...
function coordinatesFromGoogleURL(
  url: URL,
...
function coordinatesFromAppleURL(
  url: URL,
): { latitude: number; longitude: number } | null {
  ...
  if (coordinateMatch) {
    return coordinatesFromParts(coordinateMatch[1], coordinateMatch[2]);
  }
...
function placeNameFromGoogleURL(url: URL): string | null {
  ...
  const placeIndex = parts.findIndex((part) =>
    part === "place" || part === "search"
  );
...
function placeNameFromAppleURL(url: URL): string | null {
  ...
  if (placeIndex >= 0 && parts[placeIndex + 1]) {
    ...
    return isCoordinateText(pathName) || looksLikeStreetAddress(pathName)
      ? null
      : pathName;
  }
  ...
  return isCoordinateText(title) || looksLikeStreetAddress(title)
    ? null
    : title;
}
...
function looksLikeStreetAddress(value: string): boolean {
  return /\b\d{1,6}\s+[^,]+\b(st|street|ave|avenue|blvd|boulevard|rd|road|dr|drive|ln|lane|way|ct|court|pl|place|pkwy|parkway|hwy|highway)\b/i
    .test(value);
}
...
async function enrichCandidateCategory(
  ...
  if (!shouldRunAICategoryClassifier(candidate)) {
    return candidate;
  }

  const suggestion = await aiCategorySuggestion(candidate, source, steps);
  if (!suggestion) {
    return candidate;
  }

  const shouldApplyCategory = shouldApplyAICategory(
    candidate.category,
    suggestion,
  );
  steps.push(
    shouldApplyCategory ? "ai_category_applied" : "ai_category_recorded",
  );
  ...
  return {
    ...
    category_confidence: shouldApplyCategory
      ? roundConfidence(suggestion.confidence)
      : candidate.category_confidence,
  };
}

function shouldRunAICategoryClassifier(candidate: ExtractedCandidate): boolean {
  const mode = firstNonEmpty([
    Deno.env.get("WANDER_AI_CATEGORY_MODE"),
    Deno.env.get("WANDER_OPENAI_CATEGORY_MODE"),
  ])?.toLowerCase() ?? "ambiguous";
  return mode === "all" || candidate.category === "place";
}

async function aiCategorySuggestion(
  candidate: ExtractedCandidate,
  source: SourceArtifact,
  steps: string[],
): Promise<AICategorySuggestion | null> {
  const provider = configuredAIProviderName();
  steps.push(`ai_provider_${provider}`);

  const result = await aiCategoryResult(candidate, source);
  if (!result.ok) {
    if (isAIProviderUnavailable(result.errorCode)) {
      steps.push("ai_category_skipped_no_provider");
    } else {
      steps.push("ai_category_failed");
    }
    console.warn(
      "ai_category_classification_failed",
      `${result.provider}:${result.errorCode}:${result.message}`,
    );
    return null;
  }

  steps.push("ai_category_lookup");
  return result.value;
}

function aiCategoryResult(
  candidate: ExtractedCandidate,
  source: SourceArtifact,
): Promise<StructuredJSONResult<AICategorySuggestion>> {
  return structuredJSON({
    task: "place_category",
    schemaName: "recme_place_category",
    ...
    legacyOpenAIModelEnvKeys: ["WANDER_OPENAI_CATEGORY_MODEL"],
    timeoutEnvKeys: [
      "WANDER_AI_CATEGORY_TIMEOUT_MS",
      "WANDER_OPENAI_CATEGORY_TIMEOUT_MS",
    ],
    defaultTimeoutMS: 3_500,
    defaultModels: {
      openai: "gpt-5.4-nano",
    },
    validate: validateCategorySuggestion,
  });
}
...
function categoryJSONSchema(): Record<string, unknown> {
  return {
    type: "object",
    properties: {
      category: {
...
function isAIProviderUnavailable(errorCode: string): boolean {
  return errorCode === "model_unavailable" ||
    errorCode === "provider_unavailable" ||
    errorCode === "unsupported_provider";
}

function validateCategorySuggestion(
  value: unknown,
  ...
  const category = "category" in value && typeof value.category === "string"
    ? value.category
    : null;
  const subcategory =
    "subcategory" in value && typeof value.subcategory === "string"
      ? value.subcategory
      : "";
  const confidence =
...
async function authenticatedRpc<T>(
  ...
  const anonKey = Deno.env.get("SUPABASE_ANON_KEY") ??
    Deno.env.get("WANDER_SUPABASE_ANON_KEY");
...
async function supabaseRpc<T>(
  ...
  if (!response.ok) {
    throw new Error(
      `rpc_${name}_failed:${response.status}:${await response.text()}`,
    );
  }
