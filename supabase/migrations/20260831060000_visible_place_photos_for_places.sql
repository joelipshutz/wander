...
-- Map presentation groups equivalent provider/coordinate place rows into one
-- visible place. Keep the database rows distinct, but rank their eligible
-- photos as one cursor-paginated gallery for that grouped surface.
create or replace function public.visible_place_photos_for_places(
...
as $$
  ...
  eligible_photos as materialized (
    ...
    join requested_places requested on requested.place_id = user_place.place_id
    join public.profiles contributor on contributor.id = user_place.user_id
    where user_place.visibility = 'followers'
...
revoke all on function public.visible_place_photos_for_places(uuid[], timestamptz, integer, uuid, integer)
  from public, anon;
grant execute on function public.visible_place_photos_for_places(uuid[], timestamptz, integer, uuid, integer)
  to authenticated;
