...
as $$
  select case
    when normalized in ('coffee', 'coffee shop', 'cafe', 'espresso', 'roaster', 'bakery', 'tea shop') then 'coffee'
    when normalized similar to '%(restaurant|taqueria|ramen|sushi|pizza|diner|kitchen|grill|noodle|taco|food market|fast food)%' then 'restaurant'
    when normalized similar to '%(bar|brewery|winery|cocktail|pub|nightlife)%' then 'bar'
    when normalized similar to '%(hike|trail|waterfall|hot spring|canyon|mountain|observatory)%' then 'hike'
    when normalized similar to '%(park|playground|garden|plaza|beach|lake|national park)%' then 'park'
    when normalized similar to '%(pilates|reformer|lagree)%' then 'pilates studio'
    when normalized similar to '%(fitness studio|yoga|barre|wellness studio|stretch studio)%' then 'fitness studio'
    when normalized similar to '%(gym|fitness center|training|strength|workout|climbing gym|boxing gym)%' then 'gym'
    when normalized similar to '%(spiritual|church|temple|shrine|mosque|synagogue|chapel|cathedral|meditation)%' then 'spiritual'
    when normalized similar to '%(hospital|urgent care|medical center|health center|clinic)%' then 'hospital'
    when normalized similar to '%(pharmacy|drugstore)%' then 'pharmacy'
    when normalized similar to '%(veterinarian|veterinary|animal hospital|animal service|pet clinic|pet hospital)%' then 'veterinarian'
    when normalized similar to '%(hotel|motel|resort|lodging|boutique hotel|[345][ -]star hotel)%' then 'hotel'
    when normalized similar to '%(transportation|transit|airport|train station|bus station|ferry|subway|station)%' then 'transportation'
    when normalized similar to '%(shop|store|art supply store|mall|boutique|market)%' then 'shop'
    else 'place'
  end
  from (
    select trim(regexp_replace(lower(coalesce(input_value, '')), '[^a-z0-9&/ -]+', ' ', 'g')) as normalized
  ) normalized_input;
$$;
...
as $$
  select case
    when primary_category = 'place' then null
    when nullif(trim(coalesce(input_value, '')), '') is null then app.place_default_subcategory(primary_category)
    when app.place_primary_category(input_value) = primary_category
      and trim(lower(input_value)) = primary_category then app.place_default_subcategory(primary_category)
...
alter table public.places
  ...
  add column if not exists raw_provider_type text;

alter table public.user_places
  add column if not exists category_override text,
  add column if not exists subcategory_override text,
  add column if not exists category_override_source text,
  add column if not exists category_override_confidence double precision;

update public.places
set
  raw_provider_type = coalesce(raw_provider_type, category),
  primary_category = coalesce(primary_category, app.place_primary_category(category)),
  subcategory = coalesce(subcategory, app.place_subcategory(category, app.place_primary_category(category))),
  category_source = coalesce(nullif(category_source, ''), 'legacy'),
  category_confidence = coalesce(category_confidence, confidence)
where primary_category is null
   or raw_provider_type is null
   or category_source is null
   or (subcategory is null and app.place_primary_category(category) <> 'place');

update public.places
set category = primary_category
where primary_category is not null
  and category is distinct from primary_category;

update public.places
set
  primary_category = coalesce(primary_category, app.place_primary_category(category), 'place'),
  raw_provider_type = coalesce(nullif(raw_provider_type, ''), category, primary_category, 'place'),
  category_source = case
    when category_source in ('provider', 'deterministic', 'ai', 'legacy', 'unknown') then category_source
    else 'legacy'
  end,
  category_confidence = case
    when category_confidence is null then null
    else greatest(0, least(category_confidence, 1))
  end;
...
set
  ...
  category_override_source = case
    when category_override is null then null
    when category_override_source in ('user', 'ai', 'deterministic', 'unknown') then category_override_source
    else 'user'
  end,
  category_override_confidence = case
    when category_override_confidence is null then null
    else greatest(0, least(category_override_confidence, 1))
  end;

alter table public.places
  alter column primary_category set default 'place',
  alter column primary_category set not null,
  alter column raw_provider_type set default 'place',
  alter column raw_provider_type set not null;

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'places_primary_category_allowed'
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'places_category_source_allowed'
      and conrelid = 'public.places'::regclass
  ) then
    alter table public.places
      add constraint places_category_source_allowed
      check (category_source in ('provider', 'deterministic', 'ai', 'legacy', 'unknown'));
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'places_category_confidence_range'
      and conrelid = 'public.places'::regclass
  ) then
    alter table public.places
      add constraint places_category_confidence_range
      check (category_confidence is null or category_confidence between 0 and 1);
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'places_raw_provider_type_present'
      and conrelid = 'public.places'::regclass
  ) then
    alter table public.places
      add constraint places_raw_provider_type_present
      check (btrim(raw_provider_type) <> '');
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'user_places_category_override_allowed'
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'user_places_category_override_source_allowed'
      and conrelid = 'public.user_places'::regclass
  ) then
    alter table public.user_places
      add constraint user_places_category_override_source_allowed
      check (
        category_override_source is null
        or category_override_source in ('user', 'ai', 'deterministic', 'unknown')
      );
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname = 'user_places_category_override_confidence_range'
      and conrelid = 'public.user_places'::regclass
  ) then
    alter table public.user_places
      add constraint user_places_category_override_confidence_range
      check (
        category_override_confidence is null
        or category_override_confidence between 0 and 1
      );
  end if;
end $$;
...
begin
  ...
  if nullif(input_user_place->>'rating_score', '') is not null then
    input_rating_score := (input_user_place->>'rating_score')::smallint;
  end if;

  if input_rating_score is not null and (input_rating_score < 1 or input_rating_score > 5) then
