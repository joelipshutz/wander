-- Exclude Joe and Ryan only from analytics, including the zero-count denominator.
-- Preserve stable SECURITY DEFINER functions, pinned paths, and service-role-only
-- access from 20260815212000. This changes no notification delivery or user data.
begin;
...
begin
  with eligible_recipients as (
    ...
    where token.is_active
      ...
      and token.user_id not in (
        'user_3EhATWssjvHxwGiUaoWR5VTgeoy',
        'user_3EsQ6OZGVoIBhjfDUUfDhpa0PLc'
      )
  ), per_recipient as (
    select
      eligible.user_id,
...
comment on function public.notification_operations_snapshot(integer) is
  'Service-role-only aggregate notification frequency snapshot excluding Joe and Ryan recipients. Returns no recipient identifiers or notification content.';
