...
-- Keep the established Feed security boundary intact while making Featured
-- social proof reflect every distinct followed account that has saved a place.
-- The newest event remains the ranking signal and names the lead recommender.
...
begin
  ...
  return (
    ...
    featured_support as (
      select
        event.place_id,
        count(distinct event.actor_user_id)::integer as supporter_count
      from eligible_events event
      where event.place_id is not null
      group by event.place_id
    ),
    rendered_featured as (
      select distinct on (event.place_id)
        ...
        place_projection.place_json,
        actor.display_name,
        support.supporter_count
      from eligible_events event
      join public.profiles actor on actor.id = event.actor_user_id
      join featured_support support on support.place_id = event.place_id
      join lateral (
        ...
        (
          select jsonb_agg(
            jsonb_build_object(
              'place', place_json,
              'reason', case
                when supporter_count = 1 then format('Saved by %s', display_name)
                when supporter_count = 2 then format('Saved by %s and 1 other', display_name)
                else format('Saved by %s and %s others', display_name, supporter_count - 1)
              end
            )
            order by occurred_at desc, id desc
          )
...
revoke all on function app.followed_feed(text, integer) from public, anon, authenticated;
...
comment on function app.followed_feed(text, integer) is
  'Private Feed projection. Applies current follow, block, privacy, list, and place visibility before returning activity and Featured social proof.';
