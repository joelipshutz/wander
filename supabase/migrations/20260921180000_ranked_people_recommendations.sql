...
language sql stable security invoker set search_path = pg_catalog, public, app as $$
  with viewer as (
    ...
    select p.*, coalesce(settings.suggestion_priority, 0) as curated_priority,
      ...
      (select count(distinct vf.followed_user_id)::integer
       from public.follows vf join public.follows cf
         on cf.follower_user_id=vf.followed_user_id and cf.followed_user_id=p.id
       join public.profiles mutual on mutual.id=vf.followed_user_id
         and mutual.deleted_at is null and not mutual.is_private_profile
       where vf.follower_user_id=viewer.id and not app.is_blocked(viewer.id,mutual.id)) as shared_follow_count
    ...
    select c.*,
      ...
      + least(shared_follow_count,5)*12 + case when follows_viewer then 35 else 0 end
    ...
    select s.*, row_number() over(order by score desc, shared_follow_count desc,
  ...
  select r.id,r.handle,r.display_name,r.avatar_url,r.bio,r.home_area,r.created_at,
    ...
    r.shared_follow_count,r.position
