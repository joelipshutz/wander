...
function createTapeRenderer(canvas) {
  ...
  const frag=`precision highp float;
    varying vec2 uv;uniform sampler2D frame;uniform vec2 resolution;uniform float time;uniform float damage;uniform float motion;uniform vec2 launchFault;
    ...
    void main(){
      ...
      float loopTime=mod(t,8.);float hitA=step(1.68,loopTime)*(1.-step(1.88,loopTime));float hitB=step(4.72,loopTime)*(1.-step(4.88,loopTime));float hitC=step(6.93,loopTime)*(1.-step(7.18,loopTime));float event=max(hitA,max(hitB,hitC))*motion;if(launchFault.x>=0.)event=launchFault.x*motion;float epoch=floor(t*2.);
      float center=.16+.69*noise(vec2(epoch,2.8));if(launchFault.x>=0.)center=launchFault.y;float damageBand=band(p.y,center,.025+d*.055);
  ...
  const u=Object.fromEntries(['resolution','time','damage','motion','launchFault'].map(n=>[n,gl.getUniformLocation(program,n)]));
  return (image,t,d,still,launchFault=[-1,0])=>{gl.viewport(0,0,canvas.width,canvas.height);gl.useProgram(program);gl.bindTexture(gl.TEXTURE_2D,texture);gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,gl.RGBA,gl.UNSIGNED_BYTE,image);gl.uniform2f(u.resolution,canvas.width,canvas.height);gl.uniform1f(u.time,t);gl.uniform1f(u.damage,d);gl.uniform1f(u.motion,still?0:1);gl.uniform2f(u.launchFault,launchFault[0],launchFault[1]);gl.drawArrays(gl.TRIANGLES,0,6);};
}
