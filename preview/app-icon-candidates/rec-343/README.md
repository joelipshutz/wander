...
`source/recme-warm-map-original-source-1254.png` is a byte-for-byte copy of the initially supplied image. `source/recme-warm-map-selected-source-1254.png` is the selected revision with eight less-symmetrical palms contained inside the left sage-green strip. The selected candidate's Icon Composer source is a 1024 px normalization of that revision.
