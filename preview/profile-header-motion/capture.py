#!/usr/bin/env python3
"""Record the four motion options on a dedicated simulator, using bundled fixtures."""
import argparse
...
parser.add_argument('--variants', nargs='+', choices=['solid', 'blur', 'leading', 'compact'], default=['solid', 'blur', 'leading', 'compact'])
parser.add_argument('--roles', nargs='+', choices=['owner', 'member'], default=['owner', 'member'])
...
sim('ui', args.simulator, 'appearance', args.appearance)
...
for role in args.roles:
    for variant in args.variants:
        stem = f'{role}-{variant}-{args.appearance}' + ('-compact' if args.compact else '')
