...
function screenFrame({ mode, content, overlay = '' }) {
  return `
    <section class="ios-screen">
      <div>${statusBar()}</div>
      <main class="screen-body">
        <div class="screen-inner">
          <div class="screen-title-row"><h2>Discover</h2><button class="avatar-button" aria-label="Open profile">JL</button></div>
          ${modeTabs(mode)}
          ${content}
        </div>
      </main>
...
function wireScreenInteractions() {
  $$('.mode-tab', $('#phone-canvas')).forEach(button => {
    button.addEventListener('click', () => renderScreen(button.dataset.mode === 'people' ? 'people-populated' : 'places-populated'));
  });
