...
const table = query => ({kind: 'HogQLQuery', query, ...(query.includes('{filters}') ? {filters: {properties: []}} : {})});
...
export const permissionSections = [
  {title:'Permission enablement',body:'Population metrics in this section cover all external users; username filtering applies to the delivery section below. Current enablement comes from system observations on the new app build. Unknown is separate from denied. Limited contacts/quiet notifications count as enabled, with limited counts visible. Photo permission means saving images; the system photo picker does not request full-library access.',insightKeys:permissionInsights.slice(0,4).map(x=>x.key)},
  {title:'Notification delivery by username',body:'Use Add filter → Event properties → username → equals, then type or choose a username. The detail charts appear when exactly one account matches; the directory lists available accounts. Counts refresh every 15 minutes and cover the last 30 UTC calendar days. “Accepted by APNs” means Apple accepted the request, not proof of banner delivery. No notification content or device tokens are exported.',insightKeys:permissionInsights.slice(4).map(x=>x.key)},
];
