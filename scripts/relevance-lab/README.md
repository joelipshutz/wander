...
The fresh-checkout test command uses checked-in synthetic fixtures and needs no
credentials or ignored artifacts. The committed historical real Search/Featured
scorecards are evidence summaries: reproducing their exact numeric results
requires the original private pool keys and judgments, which are intentionally
not committed. Do not claim that passing the fixture tests reproduces those
historical scores.
...
This reads the hosted corpus in a read-only transaction and writes ignored local artifacts under `scripts/relevance-lab/output/`: a blind click-to-grade HTML page, a Markdown fallback, and the machine scoring key. The embeddings match the production document exactly: canonical name, category, subcategory, and coarse locality/region. Approved aggregated structured tags may inform the lexical and explicit reranking paths, but never the semantic document. Notes, free-text answers, personal labels, profile identity, addresses, photos, emails, and coordinates are neither selected nor sent to the embedding provider.
...
The scorer requires one response for every pooled candidate and reconstructs each hidden top-five ranking from the machine key. Search scores accept only `0`–`3`; incomplete Search coverage blocks scoring. The separate Featured scorer below supports `X` for unknown judgments. The scorer reports nDCG@5 plus simple top-result and wrong-result guardrails. Scoring is entirely local; the judgments and machine key remain ignored.
