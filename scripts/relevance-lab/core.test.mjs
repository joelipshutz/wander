...
import {
  buildLexicalPlaceDocument,
  buildSemanticPlaceDocument,
  cosineSimilarity,
...
test("semantic documents match the minimized production contract", () => {
  const place = {
    name: "Quiet Coffee",
    ...
    city: "CA",
    description: "Private prose must stay out",
    tags: ["date night", "friend favorite"],
  };

  assert.equal(
    buildSemanticPlaceDocument(place),
    "Quiet Coffee | coffee_tea_sweets | coffee_shop | Santa Monica | CA",
  );
  assert.match(buildLexicalPlaceDocument(place), /Private prose must stay out/);
  assert.match(buildLexicalPlaceDocument(place), /friend favorite/);
});
