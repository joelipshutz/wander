...
final class NavigationContractTests: XCTestCase {
    ...
    func testAddOwnsPlaceImportsAndOnlyRendersReviewForPendingItems() throws {
        ...
        XCTAssertTrue(root.contains("[.height(importHubRestingHeight), .large]"))
        XCTAssertTrue(root.contains("selection: $importHubDetent"))
        XCTAssertTrue(root.contains(".id(importHubPresentationID)"))
        XCTAssertTrue(root.contains("importHubPresentationID = UUID()"))
        XCTAssertTrue(root.contains("importHubDetent = .height(importHubRestingHeight)"))
