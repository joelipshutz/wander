...
final class NavigationContractTests: XCTestCase {
    ...
    func testCanonicalSaveDetailsStayCompactAndCollapseNotesWithOptionalQuestions() throws {
        ...
        XCTAssertTrue(mapScreen.contains("HStack(alignment: .top, spacing: WanderTheme.spacing2)"))
        ...
        XCTAssertTrue(mapScreen.contains("@State private var didSelectStatus = false"))
        XCTAssertEqual(mapScreen.components(separatedBy: "didSelectStatus = true").count - 1, 2)
        XCTAssertTrue(mapScreen.contains("if step == .confirm && !didSelectStatus"))
