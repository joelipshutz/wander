...
enum AppEntryStateResolver {
    static func signedInState(
        ...
        return .onboarding(
            ...
            step: hasRequiredIdentity(remoteProfile, usesAppleSignIn: session.isAppleSignIn == true) ? localState.nextStep : .identity
        )
    }
...
final class AppEntryCoordinator: ObservableObject {
    ...
    private let forcedLocalSimulatorOnboardingStep: OnboardingStep?
    ...
    init(
        ...
        forcedLocalSimulatorOnboardingStep: OnboardingStep? = nil
    ) {
        self.auth = auth
        ...
        self.forcedLocalSimulatorOnboardingStep = forcedLocalSimulatorOnboardingStep
    }
    ...
    private func resolve(_ authState: AuthState, forceRemote: Bool) async {
        ...
        case .signedIn(let session):
            ...
            if usesLocalSimulatorTestSession {
                resolvedUserID = session.userID
                analytics.identify(userID: session.userID)
                if let forcedLocalSimulatorOnboardingStep {
                    state = .onboarding(
                        session: session,
                        step: forcedLocalSimulatorOnboardingStep
                    )
                } else {
                    state = .ready(session: session, firstVisitWalkthroughEligible: false)
                }
            ...
            do {
                ...
                if profile?.onboardingCompletedAt != nil {
                    ...
                    if AppEntryStateResolver.hasRequiredIdentity(profile, usesAppleSignIn: session.isAppleSignIn == true) {
