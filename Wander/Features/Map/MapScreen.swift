...
enum MapSearchPerformancePolicy {
    static func rankingOrigin(mapRegion: MKCoordinateRegion) -> CLLocation {
        CLLocation(
    ...
    static func distanceOrigin(
        ...
        viewerLocation ?? rankingOrigin(mapRegion: mapRegion)
    }
...
/// Explicit search is broader than the currently rendered map source. The
/// store has already applied visibility/block policy; only the user's active
/// More refinements are allowed to narrow this corpus.
enum MapSearchCorpusPolicy {
    static func savedPlaces(
        from authorizedVisiblePlaces: [VisiblePlace],
        refinements: MapMoreFilterSelection
    ) -> [VisiblePlace] {
        MapFilterSelection.applying(refinements, to: authorizedVisiblePlaces)
    }
}
...
enum MapSearchQueryPolicy {
    ...
    static func hasLexicalMatch(in names: [String], query: String) -> Bool {
        names.contains { hasNameMatch($0, query: query) }
    }

    static func isExactNameMatch(_ name: String, query: String) -> Bool {
        lexicalScore(forName: name, query: query) == 1_000
    }

    static func hasNameMatch(_ name: String, query: String) -> Bool {
        lexicalScore(forName: name, query: query) > 0
    }
...
enum MapProviderSuggestionDestination {
    case saved(VisiblePlaceGroup)
    case candidate(PlaceCandidate)
}

enum MapProviderSuggestionPolicy {
    static func destination(
        for candidate: PlaceCandidate,
        in authorizedSavedCorpus: [VisiblePlace],
        currentUserID: String
    ) -> MapProviderSuggestionDestination {
        guard let matchingPlace = authorizedSavedCorpus.first(where: {
            VisiblePlaceGrouping.matches($0, candidate: candidate)
        }), let matchingGroup = VisiblePlaceGrouping.matchingGroup(
            for: matchingPlace,
            in: authorizedSavedCorpus,
            currentUserID: currentUserID
        ) else {
            return .candidate(candidate)
        }

        return .saved(matchingGroup)
    }
}
...
struct MapProviderResultDeduper {
...
struct MapSearchAuthorizationContext: Equatable {
    let storeRevision: UInt64
    let currentUserID: String
}

private struct MapSearchAuthorizationChangeModifier: ViewModifier {
    let context: MapSearchAuthorizationContext
    let onChange: () -> Void

    func body(content: Content) -> some View {
        content.onChange(of: context) { _, _ in
            onChange()
        }
    }
}
...
enum MapWalkthroughMemoryPolicy {
    ...
    @MainActor
    static func tutorialFallback(
        candidate: PlaceCandidate,
        snapshot: FirstVisitTutorialMemorySnapshot?,
        owner: LocalProfile
    ) -> VisiblePlace {
        let resolvedSnapshot = snapshot ?? FirstVisitTutorialMemorySnapshot(
            candidate: candidate,
            status: .been,
            date: Calendar.current.date(byAdding: .day, value: -4, to: .now) ?? .now,
            ratingScore: 5,
            note: "A peaceful neighborhood park with room to slow down and breathe.",
            tag: "good walk"
        )
        let placeID = "walkthrough_place_\(candidate.id)"
        let userPlaceID = "walkthrough_up_\(candidate.id)"
        let place = LocalPlace(
            ...
            latitude: candidate.latitude ?? 34.00585,
            longitude: candidate.longitude ?? -118.4842,
        ...
        let userPlace = LocalUserPlace(
            ...
            note: resolvedSnapshot.note,
            ratingScore: resolvedSnapshot.status == .been
                ? resolvedSnapshot.ratingScore
                : nil,
            recommendedScore: resolvedSnapshot.status == .been
                ? resolvedSnapshot.ratingScore
                : nil,
            recommendedCount: resolvedSnapshot.status == .been ? 1 : 0,
            nearbyConfirmed: true,
            visitedAt: resolvedSnapshot.status == .been ? resolvedSnapshot.date : nil,
            savedAt: resolvedSnapshot.date,
            plannedDate: resolvedSnapshot.status == .wannaGo ? resolvedSnapshot.date : nil,
            sourceType: "walkthrough",
            syncState: .localOnly
        )
        let attributes: [LocalPlaceAttribute]
        if resolvedSnapshot.tag.isEmpty {
            attributes = []
        } else {
            let encodedTag = (try? String(
                data: JSONEncoder().encode([resolvedSnapshot.tag]),
                encoding: .utf8
            )) ?? "[]"
            attributes = [
                LocalPlaceAttribute(
                    localID: "walkthrough_attr_\(candidate.id)",
                    userPlaceID: userPlace.id,
                    questionKey: "park_tags",
                    valueType: "multi_tag",
                    valueJSON: encodedTag,
                    syncState: .localOnly
                )
            ]
        }
...
struct NativeMapAnnotationDescriptor: Equatable {
    ...
    static func == (
        ...
        lhs.id == rhs.id
            ...
            && lhs.animatesEntrance == rhs.animatesEntrance
...
struct MapScreen: View {
    ...
    private var mapSearchAuthorizationContext: MapSearchAuthorizationContext {
        MapSearchAuthorizationContext(
    ...
    /// Featured/Friends/You, viewport, Featured's Been-only rule, and its
    /// presentation caps. Active More refinements remain intentional filters.
    private var mapSearchSavedCorpus: [VisiblePlace] {
    ...
    var body: some View {
        ...
        let annotationCount = annotationGroups.count + mappableSearchCandidates.count
        let activePinCoordinate: CLLocationCoordinate2D? = {
            ...
            if let selectedSearchCandidate,
               let latitude = selectedSearchCandidate.latitude,
               let longitude = selectedSearchCandidate.longitude {
        ...
        let nativeSavedAnnotations: [NativeMapAnnotationDescriptor] = annotationGroups.enumerated().map { index, group in
            ...
            let isSelected = highlightsCompactSelection && group.key == selectedPlaceGroupKey
            return NativeMapAnnotationDescriptor(
                ...
                opacity: isSelected ? 1 : CGFloat(
                    MapPinFocusPolicy.opacity(
                        for: coordinate,
                        selectedCoordinate: highlightsCompactSelection ? activePinCoordinate : nil,
                        regionSpan: currentSearchRegion.span
                    )
                ),
                animatesEntrance: isSelected || MapPinEntranceStyle.shouldAnimate(
                    index: index,
                    totalCount: annotationCount
                ),
                entranceDelay: MapPinEntranceStyle.staggerDelay(for: index),
        ...
        let nativeSearchAnnotations: [NativeMapAnnotationDescriptor] = mappableSearchCandidates.enumerated().compactMap { index, candidate -> NativeMapAnnotationDescriptor? in
            guard let latitude = candidate.latitude,
                  let longitude = candidate.longitude
            else { return nil }
            let coordinate = CLLocationCoordinate2D(latitude: latitude, longitude: longitude)
            let isSelected = highlightsCompactSelection && candidate.id == selectedSearchCandidateID
            let entranceIndex = annotationGroups.count + index
            return NativeMapAnnotationDescriptor(
                ...
                entranceDelay: MapPinEntranceStyle.staggerDelay(for: entranceIndex),
        ...
        NavigationStack {
            ...
            .modifier(
                MapSearchAuthorizationChangeModifier(
                    context: mapSearchAuthorizationContext,
                    onChange: handleMapSearchAuthorizationChange
                )
            )
    ...
    private func handleMapSearchAuthorizationChange() {
        ...
        submittedSavedSearchGroups = MapActivePinRetention.authorizedGroups(
            submittedSavedSearchGroups,
            within: authorizedPlaces,
            currentUserID: store.currentUser.id
        )
    ...
    private func selectVisiblePlace(
        _ visiblePlace: VisiblePlace,
        retainingGroup: VisiblePlaceGroup? = nil
    ) {
        routedVisiblePlace = visiblePlace
        routedVisiblePlaceGroup = retainingGroup
        selectedPlaceGroupKey = retainingGroup?.key
            ?? projectedGroupKey(for: visiblePlace)
            ?? VisiblePlaceGrouping.key(for: visiblePlace)
    }
    ...
    private func presentWalkthroughPlaceMemory() {
        ...
        if let visiblePlace = MapWalkthroughMemoryPolicy.preferredVisiblePlace(
            from: store.visiblePlaces(),
            tutorialUserPlaceID: walkthroughs.tutorialUserPlaceID,
            currentUserID: store.currentUser.id
        ) {
            walkthroughFallbackMemory = nil
            routedVisiblePlace = visiblePlace
        ...
        } else if let candidate = walkthroughs.tutorialCandidate {
            let fallback = MapWalkthroughMemoryPolicy.tutorialFallback(
                candidate: candidate,
                snapshot: walkthroughs.tutorialMemorySnapshot,
                owner: store.currentUser
            )
            selectedPlaceGroupKey = nil
            walkthroughFallbackMemory = fallback
            centerMap(
                latitude: fallback.place.latitude,
                longitude: fallback.place.longitude
            )
        } else {
            let fallback = MapWalkthroughMemoryPolicy.realisticFallback(
                owner: store.currentUser
            )
            selectedPlaceGroupKey = nil
            walkthroughFallbackMemory = fallback
    ...
    private func startSubmittedMapSearch(
        ...
        let authorizationContext = mapSearchAuthorizationContext
        let savedCandidates = savedMapSearchCandidates(for: requestedQuery)
        mapSearchTask = Task { @MainActor [
            savedCandidates,
            searchRegion,
            refinements,
            ...
            if requestRevision == mapSearchRevision,
               ...
               mapSearchAuthorizationContext == authorizationContext {
                mapSearchTask = nil
            }
        }
    }
    ...
    private func runMapSearch(
        ...
        searchRegion: MKCoordinateRegion,
        refinements: MapMoreFilterSelection,
        authorizationContext: MapSearchAuthorizationContext
    ) async {
    ...
    private func savedMapSearchCandidates(
        ...
        return MapSearchCandidatePolicy.savedCandidates(
            query: query,
            in: mapSearchSavedCorpus,
            currentUserID: store.currentUser.id
        )
    }
    ...
    static func shouldApplyMapSearchCompletion(
        ...
        currentQuery: String,
        isCancelled: Bool,
        requestAuthorizationContext: MapSearchAuthorizationContext,
        currentAuthorizationContext: MapSearchAuthorizationContext
    ) -> Bool {
        !isCancelled
    ...
    private func mapKitCandidates(
        ...
        if MapSearchQueryPolicy.intent(for: query) == .category {
            ...
            var providerResultDeduper = MapProviderResultDeduper()
            var items: [MKMapItem] = []
            ...
            for radius in MapAdaptiveSearchPolicy.radii(for: searchRegion) {
                ...
                do {
                    ...
                    items.append(contentsOf: providerResultDeduper.appendUnique(inRadiusItems) { item in
        ...
        if MapSearchQueryPolicy.shouldRecoverBeyondRegion(
            ...
            do {
                let neutralItems = try await mapKitItems(for: query, region: nil)
                    .filter { item in
                        MapSearchQueryPolicy.hasNameMatch(
                            item.name ?? "",
                            query: query
                        )
                    }
    ...
    #if DEBUG
    private static func usesREC352MapSearchFixtures(
        arguments: [String] = ProcessInfo.processInfo.arguments
    ) -> Bool {
    ...
    private static func rec352MapKitCandidates(for query: String) -> [PlaceCandidate] {
        switch normalized(query) {
        case "larchmont noodles":
            return [
                PlaceCandidate(
                    id: "rec352_mapkit_larchmont_noodles_ramen",
                    name: "Larchmont Noodles & Ramen",
                    category: "restaurant",
                    address: "512 N Larchmont Blvd",
                    locality: "Los Angeles",
                    region: "CA",
                    latitude: 34.0804,
                    longitude: -118.3237,
                    sourceProvider: "mapkit",
                    sourceProviderPlaceID: "rec352_mapkit_larchmont_noodles_ramen",
                    confidence: 1
                )
            ]
        case "long tables":
            return [
                PlaceCandidate(
                    id: "rec352_mapkit_long_tables_cafe",
                    ...
                    longitude: -118.2608,
                    sourceProvider: "mapkit",
                    sourceProviderPlaceID: "rec352_mapkit_long_tables_cafe",
                    confidence: 1
                )
            ]
        default:
    ...
    private func acceptSharedVisit(
        ...
        guard validatesPrivateCheckInDraft(submission, store: store),
              case .signedIn(let acceptingSession) = auth.state,
        ...
        do {
            ...
            return persistPrivateCheckInAnswers(result: SaveResult(userPlaceID: result.userPlaceID, syncState: .synced), submission: submission, visit: visit, store: store)
        } catch {
    ...
    private func removeMapSave(_ context: MapPlaceSaveContext) async -> Bool {
        ...
        case .editWant(let visiblePlace):
            ...
            if let remainingGroup = VisiblePlaceGrouping.matchingGroup(
                ...
                routedVisiblePlaceGroup = remainingGroup
    ...
    private func selectTypeaheadSuggestion(_ suggestion: MapSearchSuggestion) {
        ...
        case .mapKit(let candidate):
            ...
            case .saved(let savedGroup):
                selectVisiblePlace(
                    savedGroup.primary,
                    retainingGroup: savedGroup
                )
                selectedSearchCandidateID = nil
                mapSearchCandidates = []
                center(on: savedGroup.primary)
            case .candidate(let externalCandidate):
                routedVisiblePlace = nil
                routedVisiblePlaceGroup = nil
                selectedPlaceGroupKey = nil
                selectedSearchCandidateID = externalCandidate.id
                mapSearchCandidates = [externalCandidate]
                center(on: externalCandidate)
            }
        }
    }
    ...
    private func addTypeaheadSuggestion(_ suggestion: MapSearchSuggestion) {
        ...
        case .mapKit(let candidate):
            ...
            case .saved(let savedGroup):
                ...
                mapSearchCandidates = []
                center(on: savedGroup.primary)
                performAction(for: savedGroup.primary)
            case .candidate(let externalCandidate):
                routedVisiblePlace = nil
                routedVisiblePlaceGroup = nil
                selectedPlaceGroupKey = nil
...
private struct NativeMapView: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, MKMapViewDelegate, UIGestureRecognizerDelegate {
        ...
        private func refreshVisibleAnnotationAccessibility(in mapView: MKMapView) {
            ...
            defer {
                MapPerformanceProbe.recordNativeAccessibilityRefresh(
                    annotationCount: nativeAnnotations.count,
                    duration: ProcessInfo.processInfo.systemUptime - probeStart
                )
            }
            #endif
        ...
        private func handleLongPress(_ recognizer: UILongPressGestureRecognizer) {
            ...
            guard annotation(at: point, in: mapView) == nil else { return }
...
private final class NativeMapPinAnnotationView: MKAnnotationView {
    ...
    func configure(
        ...
        if shouldAnimateEntrance {
            ...
            animateEntrance(
                toOpacity: descriptor.opacity,
                delay: descriptor.entranceDelay,
                reduceMotion: reduceMotion
            )
            return
        }
    ...
    private func animateEntrance(
        ...
        transform = CGAffineTransform(
            translationX: 0,
            y: MapPinEntranceStyle.hiddenVerticalOffset
        ).scaledBy(
            x: MapPinEntranceStyle.hiddenScale,
            y: MapPinEntranceStyle.hiddenScale
        )
        UIView.animate(
            withDuration: MapPinEntranceStyle.springDuration,
            delay: delay,
...
private struct MapGestureObserver: UIViewRepresentable {
    ...
    let prioritizedAnnotationCoordinate: CLLocationCoordinate2D?
    ...
    final class Coordinator: NSObject, UIGestureRecognizerDelegate {
        ...
        private weak var prioritizedAnnotationView: MKAnnotationView?
        private var attachmentRetryTask: Task<Void, Never>?
        private var annotationPriorityRefreshTask: Task<Void, Never>?
        ...
        private func scheduleAnnotationPriorityRefresh() {
            annotationPriorityRefreshTask?.cancel()
            guard mapView != nil else { return }

            annotationPriorityRefreshTask = Task { @MainActor [weak self] in
                guard let self else { return }
                self.refreshAnnotationPriority()
                ...
                guard !Task.isCancelled else { return }
                self.refreshAnnotationPriority()
                self.annotationPriorityRefreshTask = nil
            }
        }
        ...
        private func refreshAnnotationPriority() {
            restorePrioritizedAnnotationView()
            guard let mapView,
                  let selectedCoordinate = observer.prioritizedAnnotationCoordinate
            else { return }
            ...
            for candidate in nearbyAnnotations {
                ...
                prioritizedAnnotationView = annotationView
                return
            }
        }

        private func restorePrioritizedAnnotationView() {
            prioritizedAnnotationView?.zPriority = .defaultUnselected
            prioritizedAnnotationView?.selectedZPriority = .defaultSelected
            prioritizedAnnotationView = nil
        }
...
enum MapPinEntranceStyle {
    static let maximumAnimatedPinCount = 12
    ...
    static func shouldAnimate(index: Int, totalCount: Int) -> Bool {
        totalCount > 0 && index >= 0 && index < min(totalCount, maximumAnimatedPinCount)
    }
}
...
private struct MapSearchSuggestion: Identifiable {
    enum Source {
        case saved(MapSearchSavedCandidate, saveStates: [MapPinSaveState])
    ...
    var emoji: String {
        switch source {
        case .saved(let savedCandidate, _):
            return savedCandidate.place.categoryEmoji
        case .mapKit(let candidate):
    ...
    static func saved(
        ...
        let visiblePlace = candidate.place
        let statusLabel = statusLabel(
            ownerName: visiblePlace.owner.displayName,
            placeCount: candidate.group.saveCount,
        ...
        return MapSearchSuggestion(
            ...
            source: .saved(candidate, saveStates: saveStates)
        )
    }
...
enum MapActivePinRetention {
    ...
    static func authorizedPlace(
        ...
        guard let retainedPlace else { return nil }
        return authorizedPlaces.first {
            $0.userPlace.id == retainedPlace.userPlace.id
        }
    }

    static func authorizedGroup(
    ...
    static func authorizedGroups(
        _ retainedGroups: [VisiblePlaceGroup],
        within authorizedPlaces: [VisiblePlace],
        currentUserID: String
    ) -> [VisiblePlaceGroup] {
        ...
        for place in authorizedPlaces {
            authorizedByUserPlaceID[place.userPlace.id] = place
        }

        var result: [VisiblePlaceGroup] = []
        var retainedUserPlaceIDs = Set<String>()
        for retainedGroup in retainedGroups {
            let refreshedPlaces = retainedGroup.places.compactMap { retainedPlace in
                authorizedByUserPlaceID[retainedPlace.userPlace.id]
            }
            for refreshedGroup in VisiblePlaceGrouping.groups(
                from: refreshedPlaces,
                currentUserID: currentUserID
            ) {
                let uniquePlaces = refreshedGroup.places.filter {
                    retainedUserPlaceIDs.insert($0.userPlace.id).inserted
                }
                guard !uniquePlaces.isEmpty else { continue }
                result.append(contentsOf: VisiblePlaceGrouping.groups(
                    from: uniquePlaces,
                    currentUserID: currentUserID
                ))
            }
    ...
    static func places(
        ...
        guard let activePlace else { return places }

        let validRetainingGroup = retainingGroup.flatMap { group in
            group.places.contains(where: {
                $0.userPlace.id == activePlace.userPlace.id
            }) ? group : nil
        }
        let retainedPlaces = validRetainingGroup?.places ?? [activePlace]
        var result = places
        var existingUserPlaceIDs = Set(places.map(\.userPlace.id))
        for place in retainedPlaces where existingUserPlaceIDs.insert(place.userPlace.id).inserted {
            result.append(place)
        }
    ...
    static func groups(
        ...
        guard let activePlace else { return groups }
        ...
        let retainedPlaces = validRetainingGroup?.places ?? [activePlace]
        let retainedUserPlaceIDs = Set(retainedPlaces.map(\.userPlace.id))
        if let existingGroup = groups.first(where: { group in
            !retainedUserPlaceIDs.isDisjoint(with: Set(group.places.map(\.userPlace.id)))
        }), Set(existingGroup.places.map(\.userPlace.id)).isSuperset(of: retainedUserPlaceIDs) {
            return groups
        }

        let activeAliases = retainedPlaces.reduce(into: Set<String>()) { aliases, place in
            aliases.formUnion(VisiblePlaceGrouping.groupingAliases(for: place.place))
        }
        guard let matchingIndex = groups.firstIndex(where: {
            !$0.aliases.isDisjoint(with: activeAliases)
        }) else {
            guard let retainedGroup = VisiblePlaceGrouping.groups(
                from: retainedPlaces,
                currentUserID: currentUserID
            ).first else { return groups }
            return groups + [retainedGroup]
        }

        let matchingGroup = groups[matchingIndex]
        var mergedPlaces = matchingGroup.places
        var mergedUserPlaceIDs = Set(mergedPlaces.map(\.userPlace.id))
        for place in retainedPlaces where mergedUserPlaceIDs.insert(place.userPlace.id).inserted {
        ...
        retainedGroups[matchingIndex] = VisiblePlaceGroup(
            key: validRetainingGroup?.key ?? matchingGroup.key,
...
struct MapPlaceSaveContext: Identifiable {
    ...
    let originalAttributes: [PlaceAttributeDraft]
    ...
    static func editWant(
        ...
        MapPlaceSaveContext(
            ...
            initialCuisine: initialCuisine(from: attributes),
            initialPhotoAttachments: [],
            originalAttributes: attributes.map { PlaceAttributeDraft(questionKey: $0.questionKey, valueType: $0.valueType, valueJSON: $0.valueJSON) }
        )
    }
...
func persistNewPlaceSaveSubmission(
    ...
    return persistPrivateCheckInAnswers(result: result, submission: submission, visit: targetVisit, store: store)
}
...
func persistScopedVisitOrWantSubmission(
    ...
    case .editVisit(_, let visit):
        ...
        return (persistPrivateCheckInAnswers(result: SaveResult(userPlaceID: updatedVisit.userPlaceID, syncState: updatedVisit.syncState), submission: submission, visit: updatedVisit, store: store), updatedVisit)
    case .editWant(let visiblePlace):
...
@MainActor
func persistPrivateCheckInAnswers(
    result: SaveResult,
    submission: MapPlaceSaveSubmission,
    ...
    guard submission.status == .been, let answers = submission.customQuestionAnswers else { return result }
    var completed = result
    guard let visit,
          submission.customQuestionOwnerID == store.currentUser.id,
          let userPlace = store.userPlaces.first(where: {
              $0.userID == store.currentUser.id && ($0.id == visit.userPlaceID || $0.localID == visit.userPlaceID)
          })
    else {
        if !answers.isEmpty { completed.localDetailsWarning = "Your check-in saved, but its private answers couldn't be stored on this device." }
        return completed
    }
    do {
        try preferences.savePrivateAnswers(
            answers, ownerUserID: store.currentUser.id,
            userPlaceID: userPlace.localID, visitID: visit.serverID ?? visit.localID
        )
    } catch {
        completed.localDetailsWarning = "Your check-in saved, but its private answers couldn't be stored on this device."
    }
    return completed
}
...
struct MapPlaceSaveModeDraft<Photo> {
    ...
    var customQuestionAnswers: [String: String] = [:]
    var customQuestionAnswersLoaded: Bool = true
}
...
struct MapPlaceSaveEditor: View {
    ...
    private var draftUpdate: PlaceSaveDraftUpdate {
        PlaceSaveDraftUpdate(
            form: PlaceSaveDraftForm(
                step: presentedStep == .details ? .details : .confirm,
                selectedAssignment: selectedAssignment,
    ...
    /// The walkthrough coordinator is the durable source of truth when the
    /// user rewinds from the automated details demo. SwiftUI may reconstruct
    /// the sheet while that transition is publishing, so rendering directly
    /// from the coordinator prevents a stale local `.details` value from
    /// trapping the user on the wrong screen.
    private var presentedStep: MapPlaceSaveStep {
        isPresentingWalkthroughStatusSelection ? .confirm : step
    }

    private var presentedHasSelectedStatus: Bool {
    ...
    private var sheetEditor: some View {
        NavigationStack {
            ScrollViewReader { walkthroughScrollProxy in
                ...
                .onChange(of: walkthroughs.requestedSurface) { _, requestedSurface in
                    guard requestedSurface == .map,
                          let result = pendingWalkthroughSaveResult
                    else { return }
    ...
    private var checkInQuestionsSection: some View {
        CheckInQuestionPanel(
            ...
            subtypeTitle: PlaceCheckInQuestionCatalog.subtypeDisplayTitle(
                categoryID: selectedAssignment.primaryCategory, subcategory: checkInQuestionSubtype
            ),
        ...
        .alert("Check-in saved", isPresented: Binding(
            ...
            Button("Done") {
                if let result = completedSaveWithWarning {
                    completedSaveWithWarning = nil
                    onSaveCompleted(result)
                }
            }
        } message: {
            Text(completedSaveWithWarning?.localDetailsWarning ?? "")
        }
    }
    ...
    private func isWalkthroughAutomating(_ target: WalkthroughTargetID) -> Bool {
        walkthroughs.activeSurface == .saveFlow
            && walkthroughs.currentStep?.target == target
    ...
    private var placeTypeSection: some View {
        ...
        return VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                .buttonStyle(.plain)
                .accessibilityIdentifier("save.placeType.category")
                ...
                if isRestaurantsFoodSelected {
                    ...
                    .buttonStyle(.plain)
                    .accessibilityIdentifier("save.placeType.cuisine")
                } else {
                    Button {
                    ...
                    .buttonStyle(.plain)
                    .accessibilityIdentifier("save.placeType.subcategory")
                }
            }
        }
        .background(astirBrandMode.recessedBackground)
    ...
    private var currentModeDraft: MapPlaceSaveModeDraft<MapPlaceSavePhotoAttachment> {
        MapPlaceSaveModeDraft(
            ...
            customQuestionAnswersLoaded: didLoadPrivateAnswers
        )
    }
    ...
    private func attributeDrafts() -> [PlaceAttributeDraft] {
        var drafts = PlaceSaveAttributePolicy.attributes(
            original: context.originalAttributes,
            answers: selectedAnswers,
            tags: unifiedTags,
    ...
    private var walkthroughAutomationTaskID: String {
        let surface = walkthroughs.activeSurface?.rawValue ?? "none"
        let target = walkthroughs.currentStep?.target.rawValue ?? "none"
        let sheetStep = presentedStep == .details ? "details" : "confirm"
        let saveAttempt = target == WalkthroughTargetID.saveSubmit.rawValue
            ? ".\(didStartWalkthroughAutoSave)"
            : ""
        return "\(surface).\(target).\(sheetStep)\(saveAttempt)"
    }

    @MainActor
    private func runWalkthroughAutomationIfNeeded(with proxy: ScrollViewProxy) async {
        guard walkthroughs.activeSurface == .saveFlow,
              presentedStep == .details,
              let target = walkthroughs.currentStep?.target
        else { return }
        ...
        case .saveRating:
            await runWalkthroughRatingDemo(for: target)
        ...
        case .saveSubmit:
            await runWalkthroughSaveDemo(for: target)
        default:
            break
        }
    }
    ...
    @MainActor
    private func runWalkthroughDateDemo(for target: WalkthroughTargetID) async {
        guard await waitForWalkthroughReadingBeat(target) else { return }

        let offset = tutorialDateOffset
        if selectedStatus == .been {
            ...
            withAnimation(reduceMotion ? nil : .smooth(duration: 0.55)) {
                visitedAt = min(selectedDate, Date.now)
            }
        } else {
            let selectedDate = Calendar.current.date(
                byAdding: .day,
            ...
            withAnimation(reduceMotion ? nil : .smooth(duration: 0.55)) {
                plannedDate = WannaGoDate.normalized(selectedDate)
            }
        }

        guard await waitForWalkthroughBeat(.milliseconds(1_200), target: target) else { return }
        completeWalkthroughTarget(target)
    }

    @MainActor
    private func runWalkthroughRatingDemo(for target: WalkthroughTargetID) async {
        guard await waitForWalkthroughReadingBeat(target) else { return }
        ...
        if reduceMotion {
            ...
            for score in [2.0, 3.5, 5.0] {
                guard await glideWalkthroughRating(to: score, target: target) else { return }
                guard await waitForWalkthroughBeat(.milliseconds(500), target: target) else { return }
            }
        }

        guard await waitForWalkthroughBeat(.milliseconds(800), target: target) else { return }
        completeWalkthroughTarget(target)
    }

    @MainActor
    private func runWalkthroughMoreOptionsDemo(
        ...
        // Dedicated note/tag coach steps can opt into the same runner later. The current
        // compact flow demonstrates both details inside this single More Options chapter.
        if !walkthroughHasDedicatedNoteOrTagSteps {
            await typeWalkthroughNote(for: target, with: proxy)
            guard isCurrentWalkthroughTarget(target) else { return }
            guard await waitForWalkthroughBeat(.milliseconds(900), target: target) else { return }
            await selectWalkthroughParkTags(for: target, with: proxy)
            guard isCurrentWalkthroughTarget(target) else { return }
        }

        guard await waitForWalkthroughBeat(.milliseconds(1_050), target: target) else { return }
        completeWalkthroughTarget(target)
    }

    @MainActor
    private func runWalkthroughNoteDemo(
        for target: WalkthroughTargetID,
        with proxy: ScrollViewProxy
    ) async {
        if !isShowingOptionalDetails {
    ...
    @MainActor
    private func runWalkthroughTagDemo(
        for target: WalkthroughTargetID,
        with proxy: ScrollViewProxy
    ) async {
        if !isShowingOptionalDetails {
    ...
    private func runWalkthroughQuestionDemo(
        ...
        // Point out the optional prompts without inventing observed answers.
        withAnimation(reduceMotion ? nil : .smooth(duration: 0.35)) {
            proxy.scrollTo(WalkthroughTargetID.saveQuestions, anchor: .center)
        }
        completeWalkthroughTarget(target)
    }
    ...
    private func selectWalkthroughParkTags(
        ...
        withAnimation(reduceMotion ? nil : .smooth(duration: 0.35)) {
            proxy.scrollTo(WalkthroughTargetID.saveTags, anchor: .center)
        }
        guard await waitForWalkthroughBeat(.milliseconds(700), target: target) else { return }
    ...
    @MainActor
    private func typeWalkthroughNote(
        for target: WalkthroughTargetID,
        with proxy: ScrollViewProxy
    ) async {
        withAnimation(reduceMotion ? nil : .smooth(duration: 0.35)) {
            proxy.scrollTo(WalkthroughTargetID.saveNote, anchor: .center)
        }
        guard await waitForWalkthroughBeat(.milliseconds(650), target: target) else { return }

        let demoNote = "A peaceful neighborhood park with room to slow down and breathe."
        note = ""
        ...
        for character in demoNote {
            guard isCurrentWalkthroughTarget(target), !Task.isCancelled else { return }
    ...
    @MainActor
    private func runWalkthroughSaveDemo(for target: WalkthroughTargetID) async {
        guard !didStartWalkthroughAutoSave else { return }
        guard await waitForWalkthroughReadingBeat(target) else { return }
        guard isCurrentWalkthroughTarget(target), !isSaving else { return }
        didStartWalkthroughAutoSave = true
        save()
    }

    private var tutorialDateOffset: Int {
        Int.random(in: 3...11)
    }

    private var walkthroughHasDedicatedNoteOrTagSteps: Bool {
        let steps = FirstVisitWalkthroughContent.stepsBySurface[.saveFlow, default: []]
        return steps.contains(where: { $0.target == .saveNote })
    ...
    @MainActor
    private func glideWalkthroughRating(
        to destination: Double,
        target: WalkthroughTargetID
    ) async -> Bool {
        let start = selectedRatingScore
        let frameCount = 28
        for frame in 1...frameCount {
            guard isCurrentWalkthroughTarget(target) else { return false }
            let linear = Double(frame) / Double(frameCount)
            let eased = linear * linear * (3 - (2 * linear))
            selectedRatingScore = start + ((destination - start) * eased)
            guard await waitForWalkthroughBeat(.milliseconds(38), target: target) else { return false }
        }
        selectedRatingScore = destination
        return true
    }

    @MainActor
    private func waitForWalkthroughReadingBeat(_ target: WalkthroughTargetID) async -> Bool {
        await waitForWalkthroughBeat(
            .milliseconds(FirstVisitWalkthroughContent.automaticReadingDelayMilliseconds(for: target)),
            target: target
        )
    }

    @MainActor
    private func waitForWalkthroughBeat(
        _ duration: Duration,
        target: WalkthroughTargetID
    ) async -> Bool {
        do {
        ...
        return isCurrentWalkthroughTarget(target)
    }

    private func isCurrentWalkthroughTarget(_ target: WalkthroughTargetID) -> Bool {
        !Task.isCancelled
            && walkthroughs.activeSurface == .saveFlow
            && walkthroughs.currentStep?.target == target
    }
    ...
    @MainActor
    private func completeWalkthroughTarget(_ target: WalkthroughTargetID) {
        guard isCurrentWalkthroughTarget(target), let step = walkthroughs.currentStep else { return }
        switch step.advance {
        case .action:
            walkthroughs.perform(target)
        case .next:
    ...
    private func save() {
        ...
        do {
            if selectedStatus == .been && didLoadPrivateAnswers {
                try CheckInQuestionPreferenceStore().validatePrivateAnswers(customQuestionAnswers, ownerUserID: store.currentUser.id)
            }
        ...
        let tutorialSnapshot: FirstVisitTutorialMemorySnapshot? = {
            guard walkthroughs.activeSurface == .saveFlow else { return nil }
            let snapshotDate = selectedStatus == .been
                ? visitedAt
                : (plannedDate ?? attemptedAt)
            return FirstVisitTutorialMemorySnapshot(
                candidate: walkthroughs.tutorialCandidate ?? context.candidate,
                status: selectedStatus,
                date: snapshotDate,
                ratingScore: selectedStatus == .been ? selectedRatingScore : nil,
                note: note,
                tag: unifiedTags.sorted().first ?? ""
            )
        }()
                ...
                if let result {
                    ...
                    if walkthroughs.activeSurface == .saveFlow,
                       walkthroughs.currentStep?.target == .saveSubmit {
                        pendingWalkthroughSaveResult = result
    ...
    private func resetWalkthroughAutoSaveForRetry() {
        ...
        didStartWalkthroughAutoSave = false
    }
    ...
    private func prepareWalkthroughTarget(
        ...
        if walkthroughs.activeSurface == .saveFlow, target == .saveStatus {
            step = .confirm
        ...
        if selectedStatus == .wannaGo {
            while let currentTarget = walkthroughs.currentStep?.target,
                  [.saveFriends, .savePhotos].contains(currentTarget) {
    ...
    private func restoreWalkthroughSavePresentationIfNeeded() {
        ...
        if target == .saveReview,
           pendingWalkthroughSaveResult == nil,
           let userPlaceID = walkthroughs.tutorialUserPlaceID {
            pendingWalkthroughSaveResult = SaveResult(
                userPlaceID: userPlaceID,
                syncState: .synced
            )
        }

        if let restoredStatus = walkthroughs.tutorialSelectedStatus {
            selectedStatus = restoredStatus
        }
        if target != .saveStatus {
            hasSelectedStatus = true
        }
        if step == .confirm,
           [
...
struct PlaceActivitySection: View {
    ...
    var body: some View {
        ...
        .alert("Couldn't load check-in", isPresented: Binding(
            get: { editLoadError != nil },
            set: { if !$0 { editLoadError = nil } }
        )) {
            Button("OK", role: .cancel) { editLoadError = nil }
        } message: {
            Text(editLoadError ?? "Connect and try Edit again.")
        }
    ...
    private func edit(_ entry: PlaceActivityEntry) {
        guard entry.canEdit, !isPreparingEdit else { return }
        isPreparingEdit = true
        Task { @MainActor in
            defer { isPreparingEdit = false }
            await prepareEdit(entry)
        }
    }

    @MainActor
    private func prepareEdit(_ entry: PlaceActivityEntry) async {
        let requestedOwnerID = store.currentUser.id
        guard entry.owner.id == requestedOwnerID else { return }
        ...
        if let wanna = entry.wanna {
            ...
            if !store.canEditVisitAnswers(visitID: visit.id) {
                guard auth.isSignedIn,
                      await store.refreshRemoteCurrentUserCalendarData(backend: backend),
                      store.currentUser.id == requestedOwnerID else {
                    editLoadError = "Your saved details couldn't be loaded. Connect and try Edit again."
                    return
                }
            }
            guard let hydrated = store.visits(for: visit.userPlaceID).first(where: {
                $0.id == visit.id || $0.localID == visit.localID || ($0.serverID != nil && $0.serverID == visit.serverID)
            }), store.canEditVisitAnswers(visitID: hydrated.id) else {
                editLoadError = "This check-in couldn't be loaded. Connect and try Edit again."
                return
            }
            context = MapPlaceSaveContext.editVisit(hydrated, visiblePlace: entry.summary.visiblePlace)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: WanderTheme.spacing2) {
            ...
            if let visit = entry.visit {
                ForEach(visitDetailValues(visit), id: \.self) { value in
                    PlaceActivityFactLine(title: value, systemImage: "checklist")
                }
                if entry.summary.visiblePlace.owner.id == store.currentUser.id {
                    CheckInPrivateAnswerSummary(
                        ownerUserID: store.currentUser.id,
                        userPlaceLocalID: entry.summary.visiblePlace.userPlace.localID,
                        visitLocalID: visit.serverID ?? visit.localID
                    )
                }
            }
